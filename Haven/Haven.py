...
def monitorar_trafego_dispositivo(ip=None, mac=None, interface=None, packet_count=100):
    ...
    """
    print(Fore.CYAN + f"[INFO] Monitorando tráfego de {ip or mac} na interface {interface or 'todas'}...")
    cap = pyshark.LiveCapture(interface=interface)
    count = 0
    try:
        for pkt in cap.sniff_continuously():
            try:
                if ip and (getattr(pkt, 'ip', None) and (pkt.ip.src == ip or pkt.ip.dst == ip)):
                    print(pkt)
                    count += 1
                elif mac and (getattr(pkt, 'eth', None) and (pkt.eth.src.lower() == mac.lower() or pkt.eth.dst.lower() == mac.lower())):
                    print(pkt)
                    count += 1
...
def consultar_mac_info():
    ...
    if not os.path.exists(known_file):
        print(Fore.RED + "[ERRO] Nenhum dispositivo conhecido salvo ainda." + Style.RESET_ALL)
        return
        ...
        if mac in relatorio:
            ...
            for bloco in blocos:
                if mac in bloco:
                    print(bloco.strip())
                    return
    # Busca básica no conjunto de conhecidos
...
def monitorar_dispositivos_rede(interface=None, intervalo=30):
    ...
    """
    print(Fore.CYAN + f"\n[INFO] Monitoramento contínuo iniciado (intervalo: {intervalo}s). Pressione Ctrl+C para parar.")
    known_devices = set()
    known_file = os.path.join(ensure_project_dir(), "dispositivos_conhecidos.pkl")
    ...
    try:
        while True:
            ...
            for ip, mac in novos:
                print(Fore.GREEN + f"[ALERTA] Novo dispositivo detectado: IP={ip} MAC={mac}" + Style.RESET_ALL)
...
[13] Monitorar dispositivos na rede
[14] Consultar informações de um MAC
[15] Monitorar tráfego de um dispositivo específico
[0] Sair
""")
...
if __name__ == "__main__":
    ...
    while True:
        ...
        if op == "1":
            ...
            count = int(input("Quantos pacotes capturar? [100]: ") or "100")
            if ":" in alvo:
                monitorar_trafego_dispositivo(mac=alvo, interface=iface if iface else None, packet_count=count)
            else:
                monitorar_trafego_dispositivo(ip=alvo, interface=iface if iface else None, packet_count=count)
        elif op == "0":
            print("Saindo...")
            break
        else:
